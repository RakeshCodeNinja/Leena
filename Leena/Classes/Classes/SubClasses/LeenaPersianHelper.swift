...
public protocol PersianSwiftCompatible {
    ...
    var lph: someType { get }
}

public extension PersianSwiftCompatible {
    var lph: LeenaPersianHelper<Self> {
        get { return LeenaPersianHelper(self) }
    }
}

public struct LeenaPersianHelper<Base> {
    var base: Base
    init(_ base: Base) {
        self.base = base
    }
...
public class PersianSwift {
    ...
    public class PersianString {
        ...
        static func IsPersianPhoneNumber(input string: String) -> Bool {
            ...
            do {
                ...
                return regex.firstMatch(in: input, options: NSRegularExpression.MatchingOptions(rawValue: 0), range: NSRange(location: 0, length: input.count)) != nil
            } catch {
                return false
            }
        }
        ...
        static func StringWithIranRialStyle(from string: String) -> String? {
            guard let str = string.lph.withCurrencyStyle else { return nil }
            return str + " ریال"
        }
        ...
        static func SortedPersianStringArray(from array: [String], desc: Bool = false) -> [String] {
            ...
            let sorted = array.sorted {
                ...
                return str1.compare(str2 as String, options: .caseInsensitive, range: NSRange(location: 0, length: str1.length), locale: local) == .orderedAscending
            }
            return sorted
        }
    ...
    public class PersianDate {
        ...
        public func getDateString(with outputFormat: OutputFormat = PersianDate.DefaultOutputFormat) -> String {
            ...
            return dateFormatter.string(from: dateObject).lph.withPersianDigits
        }
...
public extension LeenaPersianHelper where Base == Int {
    ...
    var stringWithPersianDigits: String {
        return String(describing: self.base).lph.withPersianDigits
    }
    
    var stringWithCurrencyStyle: String {
        return String(describing: self.base).lph.withCurrencyStyle!
    }
    
}

public extension LeenaPersianHelper where Base == Double {
...
public extension LeenaPersianHelper where Base == Float {
